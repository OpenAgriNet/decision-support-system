...
- **`core/channel` naming.** It currently does composition *and* channel shaping.
  `DSS_ARCHITECTURE.md` §3 lists them as separate logical functions and the
  2026-09-03 classification names "response composer" as the agent; the skeleton
  merges the two because neither exists yet. Split when the Reviewer lands, and
  check then whether channel shaping is an agent at all or plain code.
- **The three `dss-design-v2.md` conflicts in §0.** Owner: Bhavesh. The `ref` one
  blocks step 1.
- **An ADR for the agent / plain-code classification** (§0).
- **The coverage gate** (§0) — required by the pipelines, configured nowhere.
...
- Everything still open in Appendix A.6.
