...
from dss.core.moderation.models import ModerationContext, ModerationDecision
...
class TurnResult(BaseModel):
    """Both findings for a turn. The caller decides precedence — a rejecting
    ``decision`` overrides the intent downstream — but both are computed so the
    reject path costs no extra round trip later."""
...
async def run_turn(
    ...
    *,
    ...
    """Classify intent and moderate the turn concurrently, returning both."""
